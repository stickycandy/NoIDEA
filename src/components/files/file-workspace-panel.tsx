...
function DiffFileList({
  ...
  return (
    <div className="h-full flex flex-col min-h-0">
      <div className="space-y-1 border-b border-border/30 bg-card/26 px-3 py-2 backdrop-blur-md">
          ...
          {diffOutline.files.map((file) => (
            <ContextMenu key={file.key}>
              <ContextMenuTrigger asChild>
                <button
                  type="button"
                  className="group flex w-full items-center gap-2 px-3 py-1.5 text-left transition-colors hover:bg-card/28"
                  onClick={() => {
      ...
      .map((lineNumber) => ({
        ...
        options: {
          ...
          linesDecorationsClassName:
            "noidea-dirty-diff-glyph noidea-dirty-diff-added",
        },
      }))
    const modifiedDecorations = modified
      ...
      .map((lineNumber) => ({
        ...
        options: {
          isWholeLine: true,
          linesDecorationsClassName:
            "noidea-dirty-diff-glyph noidea-dirty-diff-modified",
        },
      }))
    const deletedDecorations = deleted
      ...
      .map((lineNumber) => ({
        ...
        options: {
          isWholeLine: false,
          linesDecorationsClassName:
            "noidea-dirty-diff-glyph noidea-dirty-diff-deleted",
        },
      }))
    const decorations = [
      ...addedDecorations,
