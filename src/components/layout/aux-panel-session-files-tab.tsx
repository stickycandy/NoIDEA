    ...
    <div className="space-y-3 p-3">
      {groups.map((group, groupIndex) => {
        ...
        return (
          <Collapsible
            key={groupKey}
            className="overflow-hidden rounded-xl border border-border/40 bg-card/40 text-card-foreground backdrop-blur-lg"
            open={isOpen}
            onOpenChange={(open) =>
              setOpenGroups((prev) => ({
                ...prev,
          ...
          >
            <CollapsibleTrigger asChild>
              <button
                type="button"
                className="flex w-full items-start gap-3 px-3 py-3 text-left transition-colors hover:bg-accent/24"
              >
                <span
                  className={cn(
                    "mt-0.5 inline-flex size-5 shrink-0 items-center justify-center rounded-md border border-border/40 bg-card/38 text-muted-foreground transition-colors",
                    isOpen && "bg-card/68 text-accent-foreground"
                  )}
                >
                  <ChevronRight
                    className={cn(
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-1.5">
                    <span className="rounded-md border border-border/40 bg-card/36 px-1.5 py-0.5 text-[10px] text-muted-foreground">
                      {t("changeCount", { count: group.files.length })}
                    </span>
                    <span className="rounded-md border border-border/40 bg-card/36 px-1.5 py-0.5 text-[10px] text-muted-foreground">
                      {t("fileCount", { count: uniqueFileCount })}
                    </span>
                    <span className="inline-flex items-center gap-1 rounded-md border border-border/40 bg-card/36 px-1.5 py-0.5 font-mono text-[10px] text-foreground">
                      <CommitFileAdditions
                ...
                <span className="mt-0.5 shrink-0 rounded-md border border-border/40 bg-card/36 px-1.5 py-0.5 text-[10px] tabular-nums text-muted-foreground">
                  #{groupIndex + 1}
                </span>
              </button>
            </CollapsibleTrigger>
            <CollapsibleContent className="border-t border-border/30 bg-card/24">
                  ...
                  return (
                    <li key={file.id}>
                      <button
                        type="button"
                        className={cn(
                          "flex min-w-0 w-full items-center gap-2 rounded-lg border px-2.5 py-2 text-left backdrop-blur-md",
                          isRemoved
                            ? "border-destructive/30 bg-destructive/10 cursor-not-allowed"
                            : "border-border/40 bg-card/36 transition-colors hover:bg-accent/22"
                        )}
                        disabled={isRemoved}
                        onClick={
                          isRemoved
                      ...
                      >
                        ...
                        {isRemoved ? (
                          ...
                          <span className="inline-flex shrink-0 items-center gap-1 rounded-md border border-border/40 bg-card/36 px-1.5 py-0.5 font-mono text-[10px] text-foreground">
                            <CommitFileAdditions
