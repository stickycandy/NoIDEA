...
use crate::models::{SystemAppearanceSettings, SystemLanguageSettings, SystemProxySettings};
use crate::network::proxy;

const SYSTEM_APPEARANCE_SETTINGS_KEY: &str = "system_appearance_settings";
const SYSTEM_PROXY_SETTINGS_KEY: &str = "system_proxy_settings";
const SYSTEM_LANGUAGE_SETTINGS_KEY: &str = "system_language_settings";
const APPEARANCE_SETTINGS_UPDATED_EVENT: &str = "app://appearance-settings-updated";
...
) -> Result<SystemAppearanceSettings, AppCommandError> {
    if !(20..=100).contains(&settings.window_opacity) {
        return Err(AppCommandError::configuration_invalid(
            "Window opacity must be between 20 and 100",
        ));
    }
...
pub(crate) async fn load_system_appearance_settings(
    conn: &DatabaseConnection,
) -> Result<SystemAppearanceSettings, AppCommandError> {
    let raw = app_metadata_service::get_value(conn, SYSTEM_APPEARANCE_SETTINGS_KEY)
        .await
        .map_err(AppCommandError::from)?;

    let Some(raw) = raw else {
        return Ok(SystemAppearanceSettings::default());
    };

    let parsed = serde_json::from_str::<SystemAppearanceSettings>(&raw).map_err(|e| {
        AppCommandError::configuration_invalid("Failed to parse stored appearance settings")
            .with_detail(e.to_string())
    })?;
...
#[tauri::command]
pub async fn get_system_appearance_settings(
    db: State<'_, AppDatabase>,
) -> Result<SystemAppearanceSettings, AppCommandError> {
    load_system_appearance_settings(&db.conn).await
}

#[tauri::command]
pub async fn update_system_appearance_settings(
    settings: SystemAppearanceSettings,
    db: State<'_, AppDatabase>,
    app: tauri::AppHandle,
) -> Result<SystemAppearanceSettings, AppCommandError> {
    ...
    let serialized = serde_json::to_string(&normalized).map_err(|e| {
        AppCommandError::invalid_input("Failed to serialize appearance settings")
            .with_detail(e.to_string())
    })?;

    app_metadata_service::upsert_value(&db.conn, SYSTEM_APPEARANCE_SETTINGS_KEY, &serialized)
        .await
        .map_err(AppCommandError::from)?;

    let _ = app.emit(APPEARANCE_SETTINGS_UPDATED_EVENT, &normalized);
