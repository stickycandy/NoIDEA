...
mod tests {
    ...
    fn parses_gemini_session_detail_from_chat_json() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let base: PathBuf = env::temp_dir().join(format!("noidea-gemini-test-{nanos}"));
        let chats_dir = base.join("tmp").join("noidea").join("chats");
        fs::create_dir_all(&chats_dir).expect("create chat dir");
        fs::write(
            base.join("tmp").join("noidea").join(".project_root"),
